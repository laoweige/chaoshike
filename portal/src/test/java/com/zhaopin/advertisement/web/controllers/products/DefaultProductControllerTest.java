...
public class DefaultProductControllerTest {
    ...
    private DefaultProductController controller;

    @Before
    public void setUp() {
        initMocks(this);
        controller = new DefaultProductController(api, rdApi, userSession);
    }
    ...
    public void should_get_positions_for_user() throws Exception {
        ...
        when(mockSession.getAttribute("companyName")).thenReturn("ThoughtWorks");
        ...
        assertThat(result.getViewName(), is("order/default/step2_positions"));
    ...
    public void should_get_product_form() throws Exception {
        ...
        given(sellStrategies.readEntity(List.class)).willReturn(
            new ArrayList<Map<String,Object>>(){{add(new HashMap<String,Object>(){{
                put("productId", 1);
                put("duration", 7);
                put("discount", 0.95f);
                put("unit", "day");
        ...
        Assert.assertThat(modelAndView.getViewName(), is("order/default/step3_conditions"));
